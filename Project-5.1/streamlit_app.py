    ...
    with st.form("predict_form"):
        ...
        with col1:
            ...
            LotArea = st.number_input(
                "LotArea", min_value=0, value=int(defaults.get("LotArea", 8450) or 8450)
            )
        with col2:
            LotFrontage = st.number_input(
                "LotFrontage",
                min_value=0,
                value=int(defaults.get("LotFrontage", 65) or 65),
            )
            LotShape = st.selectbox("LotShape", ["Reg", "IR1", "IR2", "IR3"])
            LandContour = st.selectbox("LandContour", ["Lvl", "Bnk", "HLS", "Low"])
        with col3:
            OverallQual = st.slider(
                "OverallQual", 1, 10, int(defaults.get("OverallQual", 5) or 5)
            )
            OverallCond = st.slider(
                "OverallCond", 1, 10, int(defaults.get("OverallCond", 5) or 5)
            )
        ...
        with col4:
            GrLivArea = st.number_input(
                "GrLivArea",
                min_value=0,
                value=int(defaults.get("GrLivArea", 1460) or 1460),
            )
            TotRmsAbvGrd = st.number_input(
                "TotRmsAbvGrd",
                min_value=0,
                value=int(defaults.get("TotRmsAbvGrd", 6) or 6),
            )
        with col5:
            BedroomAbvGr = st.number_input(
                "BedroomAbvGr",
                min_value=0,
                value=int(defaults.get("BedroomAbvGr", 3) or 3),
            )
            FullBath = st.number_input(
                "FullBath", min_value=0, value=int(defaults.get("FullBath", 2) or 2)
            )
        with col6:
            HalfBath = st.number_input(
                "HalfBath", min_value=0, value=int(defaults.get("HalfBath", 1) or 1)
            )
            KitchenAbvGr = st.number_input(
                "KitchenAbvGr",
                min_value=0,
                value=int(defaults.get("KitchenAbvGr", 1) or 1),
            )
        ...
        with col7:
            TotalBsmtSF = st.number_input(
                "TotalBsmtSF",
                min_value=0,
                value=int(defaults.get("TotalBsmtSF", 856) or 856),
            )
        with col8:
            FirstFlrSF = st.number_input(
                "1stFlrSF", min_value=0, value=int(defaults.get("1stFlrSF", 856) or 856)
            )
        ...
        with col9:
            GarageArea = st.number_input(
                "GarageArea",
                min_value=0,
                value=int(defaults.get("GarageArea", 548) or 548),
            )
        with col10:
            GarageCars = st.number_input(
                "GarageCars", min_value=0, value=int(defaults.get("GarageCars", 2) or 2)
            )
        with col11:
            GarageYrBlt = st.number_input(
                ...
                min_value=1800,
                max_value=2050,
                value=int(defaults.get("GarageYrBlt", 2000) or 2000),
            )
        ...
        with col12:
            ExterQual = st.selectbox(
                "ExterQual", ["Po", "Fa", "TA", "Gd", "Ex"], index=2
            )
            KitchenQual = st.selectbox(
                "KitchenQual", ["Po", "Fa", "TA", "Gd", "Ex"], index=2
            )
        with col13:
            BsmtQual = st.selectbox(
                "BsmtQual", ["None", "Po", "Fa", "TA", "Gd", "Ex"], index=3
            )
            HeatingQC = st.selectbox(
                "HeatingQC", ["Po", "Fa", "TA", "Gd", "Ex"], index=2
            )
        with col14:
            FireplaceQu = st.selectbox(
                "FireplaceQu", ["None", "Po", "Fa", "TA", "Gd", "Ex"], index=0
            )
            GarageQual = st.selectbox(
                "GarageQual", ["None", "Po", "Fa", "TA", "Gd", "Ex"], index=2
            )
    ...
    if submitted:
        ...
        with st.spinner("Predicting..."):
            try:
                ...
                with col_info:
                    st.metric(
                        "Prediction (log scale)", f"{result['prediction_log']:.4f}"
                    )
                ...
                if explainer is not None:
                    with st.expander(
                        "🔍 Giải thích dự đoán (Explain Prediction)", expanded=True
                    ):
                        try:
                            # Prepare the input for explanation
                            model, feature_names, transform_config, defaults = (
                                _cached_artifacts()
                            )
                            df_raw = prepare_single_record(input_dict, defaults)
                            df_encoded = run_full_processing(df_raw)
                            X_instance = df_encoded.drop(
                                columns=["SalePrice"], errors="ignore"
                            )
                            ...
                            top_15 = top_contributions.head(15).sort_values(
                                "shap_value", ascending=True
                            )

                            colors = [
                                "#ff4444" if x < 0 else "#44ff44"
                                for x in top_15["shap_value"]
                            ]
                            y_pos = np.arange(len(top_15))

                            ax.barh(
                                y_pos, top_15["shap_value"], color=colors, alpha=0.7
                            )
                            ax.set_yticks(y_pos)
                            ax.set_yticklabels(top_15["feature"], fontsize=9)
                            ax.set_xlabel(
                                "SHAP Value (Contribution to Prediction)", fontsize=11
                            )
                            ax.set_title(
                            ...
                            with col1:
                                st.metric(
                                    "Base Value", f"{explanation['base_value']:.4f}"
                                )
                            with col2:
                                total_contribution = sum(explanation["shap_values"])
                                st.metric(
                                    "Total Contribution", f"{total_contribution:.4f}"
                                )
                            with col3:
                                st.metric(
                                    "Final Prediction",
                            ...
                            display_df = top_contributions[
                                ["feature", "shap_value"]
                            ].copy()
                            display_df["contribution_%"] = (
                                display_df["shap_value"]
                                / abs(display_df["shap_value"]).sum()
                                * 100
                            ).round(2)
                            display_df.columns = [
                                "Feature",
...
def page_explain():
    ...
    if not SHAP_AVAILABLE:
        st.error(
            "⚠️ SHAP chưa được cài đặt. Vui lòng cài đặt bằng lệnh: `pip install shap`"
        )
    ...
    if explainer is None:
        st.error(
            "⚠️ Không thể load model explainer. Đảm bảo đã train model và có đầy đủ artifacts."
        )
        return
    ...
    ax.barh(
        ...
        top_features["importance_abs"],
        color=plt.cm.get_cmap("viridis")(np.linspace(0, 1, len(top_features))),
    )
    ax.set_yticks(range(len(top_features)))
    ax.set_yticklabels(top_features["feature"], fontsize=10)
    ax.set_xlabel(
        "Mean Absolute SHAP Value (Importance)", fontsize=11, fontweight="bold"
    )
    ax.set_title(
        f"Top {max_features} Most Important Features", fontsize=13, fontweight="bold"
    )
    ax.grid(axis="x", alpha=0.3)
    ...
    with col3:
        ...
        percentage = (
            (top_5_importance / total_importance * 100) if total_importance > 0 else 0
        )
