...
class FeatureEngineer:
    ...
    def engineer_quality_features(self):
        ...
        if "MasVnrArea" in self.df.columns:
            self.df["MasVnrArea"] = pd.to_numeric(
                self.df["MasVnrArea"], errors="coerce"
            ).fillna(0)
        else:
    ...
    def get_summary(self):
        ...
        dropped_features = [
            col for col in self.original_cols if col not in self.df.columns
        ]
...
if __name__ == "__main__":
    ...
    df_engineered = (
        engineer.engineer_garage_features()
